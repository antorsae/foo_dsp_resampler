...
static void apply_window(fb_sample_t * const data, const unsigned int /*size_t*/ data_len);
static void compute_autocorr(const fb_sample_t * const data, const unsigned int /*size_t*/ data_len, double * const autoc, const int m);
static int compute_lpc(const double * const autoc, double * const lpc, const int lpc_order);
static void lpc_extrapolate_data(fb_sample_t * const data0, const unsigned int /*size_t*/ data_len, const unsigned int /*size_t*/ extra, const double * const lpc, const int order, const bool invdir);
...
{
    fb_sample_t * tdata0 = new fb_sample_t[extra_bkwd + data_len + extra_fwd];    // for 1 channel only
    fb_sample_t * const tdata = tdata0 + extra_bkwd;                        // for 1 channel only

    double* autoc = new double[lpc_order+1];
    double* lpc = new double[lpc_order];
    int max_order;
    ...
    for(int c = 0; c < nch; c++)
    {
        ...
        if (extra_fwd)
        {
            ...
            for (int i = (int)data_len; i < (int)(data_len+extra_fwd); i++)
...
static void apply_window(fb_sample_t * const data, const unsigned int /*size_t*/ data_len)
{
    if (0) // subtract the mean
    {
        ...
        for(int i = 0; i < (int)data_len; i++)
            data[i] -= (fb_sample_t)mean;
    }

    if (1)    // Welch window
    {
        const fb_sample_t n2 = (data_len+1)/2.0f;
        for(int i = 0; i < (int)data_len; i++)
        {
            fb_sample_t k = (i+1-n2)/n2;
            data[i] *= 1.0f - k*k;
        }
    }
}
...
static void lpc_extrapolate_data(fb_sample_t * const data0, const unsigned int /*size_t*/ data_len, const unsigned int /*size_t*/ extra, const double * const lpc, const int order, const bool invdir)
{
    ...
    if (invdir == false)
    {
        fb_sample_t * data = data0 + data_len - order;
        for(i = 0; i < (int)extra; i++)
        {
            fb_sample_t sum = 0;
            for(j = 0; j < order; j++)
                sum -= data[i+j] * (fb_sample_t)lpc[order-1-j];
    ...
    {
        fb_sample_t * data = data0 - 1 + order;
        for(i = 0; i < (int)extra; i++)
        {
            fb_sample_t sum = 0;
            for(j = 0; j < order; j++)
