...
static __inline int lsx_rdft_init_A(int n, FFTcontext* z)
{
    if(n <= 131072)
    {
        lsx_rdft_null(z);
        ...
        return lsx_rdft_init(n, z);
}

static __inline void lsx_rdft_A(int n, int type, double * d, FFTcontext* z)
{
    if(n <= 131072)
...
int lsx_set_dft_length_new(int num_taps) /* Set to 4 x nearest power of 2 */
{      /* or half of that if danger of causing too many cache misses. */
  const int log2_dft_min_size = 10; /* Min DFT size must be in range 8 to 16 */
  double d = log((double)num_taps) / log(2.);
  return 1 << range_limit((int)(d + 2.77), log2_dft_min_size, max((int)(d + 1.77), 17));
}
...
int lsx_set_dft_length(int num_taps) /* Set to 4 x nearest power of 2 */
{
  ...
  if (result < 65536) result *= 2;
  result = range_limit(result, 2048, 131072);
  assert(num_taps * 2 < result); // increased bit accuracy can trigger this assert, but it doesn't seem to cause any issues
  return result;
}


static double lsx_kaiser_beta_old(double att, double unused)
{
...
static double lsx_kaiser_beta(double att, double tr_bw)
{
  if (att >= 60) {
...
static double * lsx_make_lpf(int num_taps, double Fc, double beta, double rho, double scale, sox_bool dc_norm)
{
  int i, m = num_taps - 1;
...
static void lsx_kaiser_params(double att, double Fc, double tr_bw, double * beta, int * num_taps)
{
    ...
    double beta)    /* <0: value will be estimated */
{
  ...
  Fc = Fs - tr_bw;
  assert(Fc - tr_bw >= 0 && tr_bw >= 0);
  lsx_kaiser_params(att, Fc, tr_bw, &beta, num_taps);
#ifdef CREATE_4X_NUMTAPS
  if (!n) {
    if (phases > 1) {
      int nnn = *num_taps / phases + 1;
      nnn = ((nnn + 3) & ~3); /* nnn is divisible by 4 */
      *num_taps = nnn * phases - 1;
    }
    else
      *num_taps = (*num_taps + modulo - 2) / modulo * modulo + 1;
  }
#else
  if (!n)
    *num_taps = phases > 1? *num_taps / phases * phases + phases - 1 : (*num_taps + modulo - 2) / modulo * modulo + 1;
#endif
  return Fn < 0? 0 : lsx_make_lpf(*num_taps, Fc, beta, rho, (double)phases, sox_false);
}
...
void lsx_fir_to_phase(double * * h, int * len, int * post_len, double phase)
{
  ...
  fftinit = lsx_rdft_init_A(work_len, &z);
  ...
  lsx_rdft_A(work_len, +1, work, &z); /* Cepstral: */
