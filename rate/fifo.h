...
#define FIFO_SIZE_T int // only signed type works with current code
#else
#define FIFO_SIZE_T int64_t // only signed type works with current code
#endif
//#endif
...
/* _mm_loadu_ps:
this intrinsic can read extra 8 bytes (for arrays of floats) so... see above */

#define RESERV 0 // 8
#define MASK (((FIFO_SIZE_T)-1)^0xf)
#define CEIL16(x) (((x)+0xf)&MASK)
#define SHIFT(x) (CEIL16(x) - (x))

typedef struct {
  char* data;
  FIFO_SIZE_T allocation;   /* Number of bytes allocated for data. */
  FIFO_SIZE_T item_size;    /* Size of each item in data */
  FIFO_SIZE_T begin;        /* Offset of the first byte to read. */
  FIFO_SIZE_T end;          /* 1 + Offset of the last byte byte to read. */
} fifo_t;

#define FIFO_MIN 0x08000
/* 0x8000 = 32kb = 8192 samples (float) or 4096 samples (double); */
...
{
    FIFO_SIZE_T old_allocation = f->allocation;
...
UNUSED static void * fifo_reserve_aligned(fifo_t * f, FIFO_SIZE_T n)
{
    ...
    for (;;)
    {
        if (CEIL16(f->end) + n <= f->allocation)
        {
            ...
            if (SHIFT(f->end))
            {
                FIFO_SIZE_T shift = SHIFT(f->end - f->begin);
...
UNUSED static __inline void * fifo_write0(fifo_t * f, FIFO_SIZE_T n)
{
  void * s = fifo_reserve(f, n);
  if (s == NULL) return NULL;
